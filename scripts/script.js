...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- State Management ---
    let state = {
        currentView: 'students', // 'students' or 'activities'
        currentLevel: 'PS', // 'PS' or 'MS'
    ...
    function renderStudentsList() {
        ...
        const filteredStudents = state.data.students
            .filter(s => s.level === state.currentLevel)
    ...
    function renderActivitiesList() {
        ...
        const filteredActivities = state.data.activities
            .filter(a => a.level === state.currentLevel)
    ...
    function renderStudentDetail(studentId) {
        ...
        const activitiesForLevel = state.data.activities.filter(a => a.level === student.level)
    ...
    function saveStudent() {
        ...
        if (state.editingStudentId && document.getElementById('studentId')) {
            // Mode édition
            currentId = document.getElementById('studentId').value; // récupère l'ID du formulaire
        ...
        if (studentDetailView.hidden) {
            render(); // refresh liste
        } else {
            showStudentDetail(currentId); // recharge la fiche avec le bon ID
        }
    }
    ...
    function deleteActivity(activityId) {
        if (confirm("Voulez-vous vraiment supprimer cette activité ? Elle sera retirée de tous les élèves.")) {
            ...
            // Remove status from all students
    ...
    function toggleActivityStatus(e) {
        ...
        // Cycle: none -> acquired -> not-acquired -> none
